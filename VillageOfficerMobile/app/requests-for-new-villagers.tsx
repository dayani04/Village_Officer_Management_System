...
export default function RequestsForNewVillagers() {
  const router = useRouter();
  const [requests, setRequests] = useState<NewVillagerRequest[]>([]);
  ...
  const fetchRequests = async () => {
    try {
      ...
      setLoading(false);
      Alert.alert('Error', err.error || 'Failed to fetch new villager requests');
    }
  };
  ...
  const handleDownload = async (filename: string) => {
    try {
      ...
      const blobData = await villagerApi.downloadNewFamilyMemberDocument(filename);
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(fileUri, {
          ...
          dialogTitle: 'Share New Villager Document',
        });
      } else {
  ...
  const renderRequestCard = (request: NewVillagerRequest, index: number) => {
    return (
        ...
        <View style={styles.cardHeader}>
          <View style={styles.requestInfo}>
            ...
            <View style={styles.requestDetails}>
              <Text style={styles.requestTitle}>New Villager Request</Text>
        ...
        <View style={styles.documentsSection}>
          ...
          {request.DocumentPath && (
            <TouchableOpacity
              ...
              onPress={() => handleDownload(request.DocumentPath!)}
            >
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>New Villager Requests</Text>
          <View style={styles.placeholder} />
        </View>
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color={Colors.primary} />
          <Text style={styles.loadingText}>Loading new villager requests...</Text>
        </View>
      </View>
    );
  }
        ...
        <View style={styles.content}>
          {requests.length === 0 ? (
            <View style={styles.noDataContainer}>
              ...
              <Text style={styles.noDataText}>No new villager requests found</Text>
              <Text style={styles.noDataSubtext}>No pending new villager registration requests</Text>
            </View>
          ) : (
            <View style={styles.cardsContainer}>
              <Text style={styles.resultsCount}>{requests.length} requests found</Text>
