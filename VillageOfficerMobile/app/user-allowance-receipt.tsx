...
import { fetchConfirmedAllowanceApplications } from '../src/api/allowanceApplication';
...
const UserAllowanceReceiptScreen: React.FC = () => {
  ...
  const loadReceipts = async () => {
    try {
      ...
      const transformedData = data.map((item: any, index: number) => ({
        ...
        Allowances_Type: item.Allowances_Type || item.allowances_type || 'N/A',
  ...
  const handleDownload = async (filename: string) => {
    try {
      ...
      const downloadUrl = `${API_URL}/allowance-applications/download/${filename}`;
