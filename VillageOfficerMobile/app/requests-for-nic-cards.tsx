...
import * as nicApplicationApi from '../src/api/nicApplication';
...
export default function RequestsForNICCards() {
  ...
  const fetchApplications = async () => {
    try {
      ...
      const pendingApplications = data.filter((app: NICApplication) => app.status === 'Pending');
  ...
  const handleStatusSelect = (villagerId: string, nicId: string, application: NICApplication) => {
    Alert.alert(
      'Status Selection',
      `Changing status for ${application.Full_Name}\n\nCurrent options:\n- PENDING\n- SEND\n- REJECTED\n- CONFIRMED`,
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Pending', onPress: () => updateStatusDirectly(villagerId, nicId, 'Pending') },
        { text: 'Send', onPress: () => updateStatusDirectly(villagerId, nicId, 'Send') },
      ]
    );
  };

  const updateStatusDirectly = async (villagerId: string, nicId: string, newStatus: string) => {
    try {
  ...
  const renderApplicationCard = (application: NICApplication) => {
    const statusKey = `${application.Villager_ID}-${application.NIC_ID}`;
      ...
      <View key={statusKey} style={styles.card}>
        ...
        <View style={styles.cardBody}>
          ...
          <View style={styles.infoRow}>
            <View style={styles.infoIcon}>
              <Ionicons name="id-card-outline" size={18} color={Colors.textLight} />
            </View>
            <View style={styles.infoContent}>
              <Text style={styles.infoLabel}>NIC Type</Text>
              <Text style={styles.infoValue}>{application.NIC_Type}</Text>
            </View>
          </View>
          ...
          <View style={styles.infoRow}>
            ...
            <View style={styles.infoContent}>
              ...
              <Text style={styles.infoValue}>{application.NIC_ID}</Text>
            </View>
          </View>
          ...
          <View style={styles.infoRow}>
            ...
            <View style={styles.infoContent}>
              ...
              <TouchableOpacity
                ...
                onPress={() => handleStatusSelect(application.Villager_ID, application.NIC_ID, application)}
              >
        ...
        <View style={styles.cardFooter}>
          ...
          <TouchableOpacity
            ...
            onPress={() => updateStatusDirectly(application.Villager_ID, application.NIC_ID, currentStatus)}
          >
        ...
        <View style={styles.content}>
          {applications.length === 0 ? (
            <View style={styles.noDataContainer}>
              ...
              <Text style={styles.noDataText}>No pending NIC applications found</Text>
              <Text style={styles.noDataSubtext}>All NIC applications have been processed</Text>
            </View>
          ) : (
