...
const HoldersAllowanceScreen: React.FC = () => {
  const router = useRouter();
  const [holders, setHolders] = useState<AllowanceHolder[]>([]);
  ...
  const fetchConfirmedApplications = async () => {
    try {
      ...
      console.log('Fetched confirmed allowance applications:', data);
      setHolders(data);
    } catch (err: any) {
  ...
  const handleViewDetails = (villagerId: string) => {
    ...
    // Navigate to allowance owner details page with query parameter
    router.push({ pathname: '/allowance-owner-details', params: { villagerId } } as any);
  };
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading allowance holders...</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Confirmed Allowance Recipients</Text>
      ...
      <View style={styles.listContainer}>
        <Text style={styles.listTitle}>All Confirmed Recipients</Text>
        {holders.length === 0 ? (
          <View style={styles.noDataContainer}>
            <Ionicons name="document-outline" size={48} color={Colors.textLight} />
            <Text style={styles.noDataText}>No confirmed allowance recipients found</Text>
            <Text style={styles.noDataSubtext}>No applications have been confirmed yet</Text>
          </View>
        ) : (
          holders.map((holder) => (
            <TouchableOpacity
              key={`${holder.Villager_ID}-${holder.Allowances_ID}`}
            ...
            >
              <View style={styles.holderInfo}>
                ...
                <View style={styles.holderDetails}>
                  <Text style={styles.allowanceType}>{holder.Allowances_Type || 'N/A'}</Text>
...
export default HoldersAllowanceScreen;
