...
import * as nicApi from '../src/api/nicApplication';
...
const SecretaryNICApplicationsScreen: React.FC = () => {
  const router = useRouter();
  const [applications, setApplications] = useState<NICApplication[]>([]);
  ...
  const loadApplications = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await nicApi.fetchNICApplications();
      const sendApplications = data.filter((app: NICApplication) => app.status === "Send");
      setApplications(sendApplications);
    } catch (err: any) {
      console.error('Error fetching NIC applications:', err);
      setError(err.error || 'Failed to fetch NIC applications');
      Alert.alert('Fetch Error', err.error || 'Failed to fetch NIC applications');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusChange = (villagerId: string, nicId: string, newStatus: string) => {
  ...
  const handleSend = async (villagerId: string, nicId: string, nicType: string, fullName: string) => {
    ...
    try {
      await nicApi.updateNICApplicationStatus(villagerId, nicId, newStatus);
  ...
  const handleDownload = async (filename: string) => {
    try {
      await nicApi.downloadDocument(filename);
  ...
  const renderApplicationItem = ({ item }: { item: NICApplication }) => {
    const key = `${item.Villager_ID}-${item.NIC_ID}`;
    ...
    return (
      <View style={styles.applicationCard}>
        <View style={styles.applicationHeader}>
          <Text style={styles.nicType}>{item.NIC_Type}</Text>
        ...
        <View style={styles.applicationDetails}>
          ...
          <Text style={styles.applicationDetail}>Application ID: {item.NIC_ID}</Text>
        </View>
            ...
            onPress={() => {
              Alert.alert(
                ...
                [
                  ...
                  { text: 'Send', onPress: () => handleStatusChange(item.Villager_ID, item.NIC_ID, 'Send') },
                  { text: 'Rejected', onPress: () => handleStatusChange(item.Villager_ID, item.NIC_ID, 'Rejected') },
                  { text: 'Confirm', onPress: () => handleStatusChange(item.Villager_ID, item.NIC_ID, 'Confirm') },
                ]
              );
            }}
            disabled={isSent}
        ...
        <View style={styles.actionContainer}>
          <TouchableOpacity
            ...
            onPress={() => handleSend(item.Villager_ID, item.NIC_ID, item.NIC_Type, item.Full_Name)}
            disabled={isSent}
          >
            <Ionicons 
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>NIC Applications (Send)</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        keyExtractor={(item) => `${item.Villager_ID}-${item.NIC_ID}`}
...
export default SecretaryNICApplicationsScreen;
