...
export default function RequestsForElections() {
  ...
  const fetchApplications = async () => {
    try {
      ...
      console.log('Fetched election applications:', data);
      const pendingApplications = data.filter((app: ElectionApplication) => app.status === 'Pending');
  ...
  const handleStatusSelect = (villagerId: string, electionrecodeID: string, application: ElectionApplication) => {
    Alert.alert(
      'Status Selection',
      `Changing status for ${application.Full_Name}\n\nCurrent options:\n- PENDING\n- SEND\n- REJECTED\n- CONFIRMED`,
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Pending', onPress: () => updateStatusDirectly(villagerId, electionrecodeID, 'Pending') },
        { text: 'Send', onPress: () => updateStatusDirectly(villagerId, electionrecodeID, 'Send') },
      ]
    );
  };

  const updateStatusDirectly = async (villagerId: string, electionrecodeID: string, newStatus: string) => {
    try {
  ...
  const renderApplicationCard = (application: ElectionApplication) => {
    const statusKey = `${application.Villager_ID}-${application.electionrecodeID}`;
      ...
      <View key={statusKey} style={styles.card}>
        ...
        <View style={styles.cardBody}>
          ...
          <View style={styles.infoRow}>
            ...
            <View style={styles.infoContent}>
              <Text style={styles.infoLabel}>Election Type</Text>
              <Text style={styles.infoValue}>{application.Election_Type}</Text>
            </View>
          </View>
          ...
          <View style={styles.infoRow}>
            <View style={styles.infoIcon}>
              <Ionicons name="bar-chart-outline" size={18} color={Colors.textLight} />
            </View>
            <View style={styles.infoContent}>
              <Text style={styles.infoLabel}>Election ID</Text>
              <Text style={styles.infoValue}>{application.electionrecodeID}</Text>
            </View>
          </View>
          ...
          <View style={styles.infoRow}>
            ...
            <View style={styles.infoContent}>
              ...
              <TouchableOpacity
                ...
                onPress={() => handleStatusSelect(application.Villager_ID, application.electionrecodeID, application)}
              >
        ...
        <View style={styles.cardFooter}>
          ...
          <TouchableOpacity
            ...
            onPress={() => updateStatusDirectly(application.Villager_ID, application.electionrecodeID, currentStatus)}
          >
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Election Applications</Text>
        ...
        <View style={styles.content}>
          {applications.length === 0 ? (
            <View style={styles.noDataContainer}>
              ...
              <Text style={styles.noDataText}>No pending election applications found</Text>
              <Text style={styles.noDataSubtext}>All election applications have been processed</Text>
            </View>
          ) : (
