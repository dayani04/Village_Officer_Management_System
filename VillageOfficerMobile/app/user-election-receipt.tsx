...
const UserElectionReceiptScreen: React.FC = () => {
  const router = useRouter();
  const [receipts, setReceipts] = useState<ElectionReceipt[]>([]);
  ...
  const handleDownload = async (filename: string) => {
    try {
      if (!filename) {
        Alert.alert('Not Available', 'Election receipt is not available for download');
        return;
      }
  ...
  const renderReceiptItem = ({ item }: { item: ElectionReceipt }) => (
    <View style={styles.receiptItem}>
      <View style={styles.receiptInfo}>
        <Text style={styles.electionType}>{item.Election_Type || 'N/A'}</Text>
        <Text style={styles.electionDate}>
          Date: {formatDate(item.electionDate)}
        </Text>
        <Text style={styles.votingPlace}>
          Place: {item.votingPlace || 'Not Assigned'}
        </Text>
      </View>
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyStateText}>No election receipts available for you</Text>
    </View>
  );
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>My Election Receipts</Text>
        </View>
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color={Colors.primary} />
          <Text style={styles.loadingText}>Loading your election receipts...</Text>
        </View>
      </View>
    );
  }
...
export default UserElectionReceiptScreen;