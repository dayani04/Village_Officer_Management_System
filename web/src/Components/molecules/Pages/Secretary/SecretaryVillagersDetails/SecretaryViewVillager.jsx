...
import './SecretaryViewVillager.css';

const SecretaryViewVillager = () => {
  const { id } = useParams();
    ...
    const fetchVillager = async () => {
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Fetch Error',
          text: err.response?.data?.error || err.message || 'Failed to fetch villager',
...
export default SecretaryViewVillager;