...
class UserAllowancesBCPage extends StatefulWidget {
  final String email;
  final String type;
  const UserAllowancesBCPage({
    Key? key,
    required this.email,
    required this.type,
  }) : super(key: key);
  ...
  @override
  State<UserAllowancesBCPage> createState() => _UserAllowancesBCPageState();
}

class _UserAllowancesBCPageState extends State<UserAllowancesBCPage> {
  PlatformFile? _file;
  bool _loading = false;
  String? _error;
  String? _success;
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final uri = Uri.parse(
        'http://localhost:5000/api/allowance-applications/',
      );
      final request = http.MultipartRequest('POST', uri)
        ..fields['email'] = widget.email
        ..fields['allowanceType'] = widget.type;
      final mime =
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Allowance Document Upload'),
        leading: IconButton(
